...
class Stage_GNN_learner(nn.Module):
    def __init__(self, nlayers, isize, osize, k, knn_metric, sparse, act, internal_type, ks, share_up_gnn, fusion_ratio,
        ...
        if self.internal_type == 'gnn':
            self.encoder1 = GCNEncoder(nfeat=isize, nhid=osize, nclass=osize, n_layers=nlayers, dropout=0,
        ...
        if self.l_n > 0:
            ...
            if self.share_up_gnn:
                self.encoder2 = GCNEncoder(nfeat=osize, nhid=osize, nclass=osize, n_layers=nlayers, dropout=self.dropout_up_gnn,
