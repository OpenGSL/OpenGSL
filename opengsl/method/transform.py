...
def normalize(mx, style='symmetric', add_loop=True):
    '''
...
if __name__ == '__main__':
    ...
    print(normalize(adj, 'row'))
    print(normalize(adj.to_dense(), 'row').to_sparse())
    print(F.normalize(adj.to_dense(), p))

    # adj = adj.coalesce()
    # inv_sqrt_degree = 1. / (torch.sqrt(torch.sparse.sum(adj, dim=1).values()) + 1e-10)
    # D_value = inv_sqrt_degree[adj.indices()[0]] * inv_sqrt_degree[adj.indices()[1]]
    # new_values = adj.values() * D_value
    # print(torch.sparse.FloatTensor(adj.indices(), new_values, adj.size()))